...
do {
    ...
    switch ( $opcao ) {
        ...
        case OPCAO_ALTERAR: alterar( $nomes ); break;
    }
...
function listarNomes( array &$nomes ) {
    ...
    sort( $nomes );
    foreach( $nomes as $chave => $n ) {
        echo "Indice: $chave -- Nome: {$n[ 'nome' ]} -- Idade: {$n[ 'idade' ]}", PHP_EOL;
    }
}
...
function alterar( &$nomes ) {

    listarNomes( $nomes );
    $indice = readline( 'Digite o indice que deseja alterar: ' );
    $nome = readline( 'Digite um nome: ' );
    $idade = readline( 'Digite a dade: ' );

    $nomes[ $indice ] = [ 'nome' => $nome, 'idade' => $idade ];
}
